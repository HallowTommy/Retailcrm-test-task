...
- Все интеграционные запросы логируются в консоль (CRM GET/POST).
- Ошибки от RetailCRM пробрасываются в ответ API в неизменном виде.
- Архитектура следует принципам SOLID:  
  - отдельный клиент `RetailCRMClient`;  
  - сервисный слой (`CustomerService`, `OrderService`, `PaymentService`);  
  - чистые Pydantic-схемы;  
  - маршруты только маршрутизируют и валидируют данные.
